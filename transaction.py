...
class Transaction:
    ...
    def __init__(self, line):
        ...
        # pdb.set_trace()
        if (info_arr[1][0:6].lower() == 'bought'):
            self.direction = 1
        elif (info_arr[1][0:4].lower() == 'sold'):
            self.direction = -1
        else:
            self.direction = None
        if (info_arr[2] != ''):
            self.quantity = int(info_arr[2])
        else:
            self.quantity = None
        self.symbol = info_arr[3]
        if (info_arr[4] != ''):
            self.price = float(info_arr[4])
        else:
            self.price = None
        if (info_arr[5] != ''):
            self.comm = float(info_arr[5])
        else:
            self.comm = None
        
        self.amt = float(info_arr[6])
